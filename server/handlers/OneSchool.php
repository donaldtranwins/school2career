...
class OneSchool{
    ...
    protected function doPreparedQuery($conn, $query, $param){
    ...
    public function processRequest(){
        ...
        if ($school = $this->doPreparedQuery($dbConn, $metadataQuery, $_GET['schid'])){
            if(is_array($school)){
                $this->output['status'] = 200;
                $this->output['school'] = $school[0];
                if ($programs = $this->doPreparedQuery($dbConn, $programsQuery,$_GET['schid']))
